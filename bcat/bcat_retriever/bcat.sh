...
#requires bsv (js), npm, xxd, curl recommends exiftool, grep
...
if [[ -p "/dev/stdin" ]]; then arrayHash="$(cat)"; else
        arrayHash="$1"; if [[ -z "$1" ]]; then echo "provide a txid as \$1"; exit 1; fi; fi
